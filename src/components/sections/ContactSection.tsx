...
export default function Contact({ profile }: { profile: Profile }) {
  ...
  return (
    <div className="h-full flex flex-col pt-12 pb-24 justify-center items-center text-center">
      ...
      <h2 className="text-xl md:text-3xl font-bold text-p3-yellow uppercase mb-16 max-w-2xl bg-p3-blue-dark/50 p-4 border border-p3-yellow/30 clip-slanted">
      ...
      <div className="flex flex-wrap gap-6 items-center justify-center w-full max-w-4xl">
        {[
          ...
          { label: "RESUME (BACKEND)", url: "/resume-backend.pdf" },
          { label: "RESUME (ML/DL)", url: "/resume-ml.pdf" }
        ].map((link, index) => (
          <motion.a
            key={index}
            href={link.url}
